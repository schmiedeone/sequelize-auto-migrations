...
const reverseModels = function(sequelize, models)
{
    ...
    for (let model in models)
    {
        ...
        if (typeof models[model].options.collate !== 'undefined')
        {
            tables[models[model].tableName].collate = models[model].options.collate;
        }
        ...
        switch (df.kind) 
        {
            ...
            case 'N':
            {
                ...
                if (df.path.length === 1)
                {
                    ...
                    if (typeof df.rhs.collate !== 'undefined') 
                    {
                        options.collate = df.rhs.collate;
                    }
